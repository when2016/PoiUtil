...
public class ExportBeanExcel<T> {
  ...
  /**
   * 这是一个通用的方法，利用了JAVA的反射机制，可以将放置在JAVA集合中并且符号一定条件的数据以 EXCEL 的形式输出
   *
   * fileName Excel文件名 sheetName sheet名称 headersName  表格属性列名数组 headersId
   * 表格属性列名对应的字段---你需要导出的字段名（为了更灵活控制你想要导出的字段） dtoList 需要显示的数据集合,集合中一定要放置符合javabean风格的类的对象 out
      ...
      for (short i = 0; i < fields.length; i++) {//遍历属性，比对
        ...
        while (zdIt.hasNext()) {//遍历要导出的字段集合
          if (zdIt.next().equals(fieldName)) {//比对JavaBean的属性名，一致就写入，不一致就丢弃
            String getMethodName = "get" + fieldName.substring(0, 1).toUpperCase() + fieldName
                .substring(1);//拿到属性的get方法
            Class tCls = l.getClass();//拿到JavaBean对象
            try {
              Method getMethod = tCls
                  .getMethod(getMethodName, new Class[]{});//通过JavaBean对象拿到该属性的get方法，从而进行操控
  ...
  */
  public static void main(String[] args) {
    ...
    list.add(new Student(1, "张三asdf", "男"));
    list.add(new Student(3, "李四asd", "男"));
    list.add(new Student(3, "王五", "女"));

    ExportBeanExcel<Student> exportBeanExcelUtil = new ExportBeanExcel();
    exportBeanExcelUtil.exportExcel("E://工单信息表bean.xls", "测试POI导出EXCEL文档", listName, listId, list);
